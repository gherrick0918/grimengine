...
describe('compendium encounter builder', () => {
  ...
  it('imports monsters from directories and files with generated slugs', async () => {
    ...
    const dirResult = await importMonsters(sourceDir, { reset: true });
    expect(dirResult.ok).toBe(true);
    expect(dirResult.added).toBe(2);
    ...
    expect(finalResult.added).toBe(1);
    expect(finalResult.count).toBe(dirResult.count + 1);
